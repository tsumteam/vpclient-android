...
@Composable
fun ProfileLoyaltyQrScreen(
    route: ProfileLoyaltyQrRoute,
    viewModel: ProfileLoyaltyQrViewModel = hiltViewModel<ProfileLoyaltyQrViewModel, ProfileLoyaltyQrViewModel.Factory>(
        creationCallback = { it.create(route) }
    )
) {
    val state by viewModel.stateFlow.collectAsStateWithLifecycle()

    ProfileLoyaltyQrScreenContent(
        state = state,
...
@Composable
private fun ProfileLoyaltyQrScreenContent(
    state: ProfileLoyaltyQrModel,
    dispatch: (ProfileLoyaltyQrIntent) -> Unit
) {
    SharedScaffold(
        topBar = {
            CenterAlignedTopAppBar(
                ...
                navigationIcon = {
                    IconButton(
                        onClick = { dispatch(ProfileLoyaltyQrIntent.CloseClick) }
                    ) {
                        Icon(
                            imageVector = Close24,
        ...
        SharedLazyColumn(
            ...
            item {
                Text(
                    text = stringResource(ClientStrings.ProfileLoyaltyQrDescription),
...
@Preview
@Composable
private fun ProfileLoyaltyQrScreenContentPreview() {
    ProfileLoyaltyQrScreenContent(
        state = ProfileLoyaltyQrModel(
            qrCode = "1234567890"
        ),
