...
@HiltViewModel
class ProfilePolicyViewModel @Inject constructor(): ClientViewModel<ProfilePolicyIntent, ProfilePolicyModel, Event>(
    ProfilePolicyModel()
) {

    override fun dispatch(intent: ProfilePolicyIntent) {
        when (intent) {
            is ProfilePolicyIntent.BackClick -> launch { ProfileStackEventManager.send(BackRoute) }
        }
    }
}
