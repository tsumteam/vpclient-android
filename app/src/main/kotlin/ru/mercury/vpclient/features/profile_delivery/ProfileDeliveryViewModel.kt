...
@HiltViewModel
class ProfileDeliveryViewModel @Inject constructor(): ClientViewModel<ProfileDeliveryIntent, ProfileDeliveryModel, Event>(ProfileDeliveryModel()) {

    override fun dispatch(intent: ProfileDeliveryIntent) {
        when (intent) {
            is ProfileDeliveryIntent.BackClick -> launch { ProfileRootEventManager.send(BackRoute) }
        }
    }
}
