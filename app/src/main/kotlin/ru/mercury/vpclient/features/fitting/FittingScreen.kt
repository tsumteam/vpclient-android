...
import ru.mercury.vpclient.shared.ui.components.fitting.CompilationCard
...
fun FittingScreen(
    ...
    ObserveAsEvents(
        ...
        when (event) {
            is FittingEvent.SnackbarErrorMessage -> {
                ...
                actions = {
                    if (state.isFittingButtonVisible) {
                        FittingIconButton(
                            ...
                            onClick = { dispatch(FittingIntent.FittingClick) }
                        )
                    }
                    ...
                    CartIconButton(
                        ...
                        onClick = { dispatch(FittingIntent.CartClick) }
                    )
                ...
                SharedLazyColumn(
                    ...
                    items(
                        ...
                        CompilationCard(
                            ...
                            onClick = { dispatch(FittingIntent.CompilationClick(compilation.id)) }
                        )
...
private fun FittingScreenContentPreview(
    @PreviewParameter(FittingModelProvider::class) state: FittingModel
) {
    FittingScreenContent(
        state = state,
        dispatch = {},
...
private class FittingModelProvider: PreviewParameterProvider<FittingModel> {
    ...
    override val values: Sequence<FittingModel> = sequenceOf(
        FittingModel(
            cartCount = 37,
            fittingCount = 3,
