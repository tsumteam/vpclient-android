...
@HiltViewModel
class ProfileGiftViewModel @Inject constructor(): ClientViewModel<ProfileGiftIntent, ProfileGiftModel, Event>(
    ProfileGiftModel()
) {

    override fun dispatch(intent: ProfileGiftIntent) {
        when (intent) {
            is ProfileGiftIntent.BackClick -> launch { ProfileStackEventManager.send(BackRoute) }
        }
    }
}
