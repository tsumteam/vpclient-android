...
@HiltViewModel
class ProfilePaymentViewModel @Inject constructor(): ClientViewModel<ProfilePaymentIntent, ProfilePaymentModel, Event>(
    ProfilePaymentModel()
) {

    override fun dispatch(intent: ProfilePaymentIntent) {
        when (intent) {
            is ProfilePaymentIntent.BackClick -> launch { ProfileStackEventManager.send(BackRoute) }
        }
    }
}
