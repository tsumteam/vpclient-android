...
fun ProfilePolicyScreen(
    viewModel: ProfilePolicyViewModel = hiltViewModel()
) {
    val state by viewModel.stateFlow.collectAsStateWithLifecycle()

    ProfilePolicyScreenContent(
        state = state,
...
@Composable
private fun ProfilePolicyScreenContent(
    state: ProfilePolicyModel,
    dispatch: (ProfilePolicyIntent) -> Unit
) {
    SharedScaffold(
        topBar = {
            CenterAlignedTopAppBar(
                ...
                navigationIcon = {
                    IconButton(
                        onClick = { dispatch(ProfilePolicyIntent.BackClick) }
                    ) {
                        Icon(
...
@Preview
@Composable
private fun ProfilePolicyScreenContentPreview() {
    ProfilePolicyScreenContent(
        state = ProfilePolicyModel(),
        dispatch = {}
    )
