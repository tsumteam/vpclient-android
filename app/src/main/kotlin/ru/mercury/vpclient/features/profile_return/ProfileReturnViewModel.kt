...
@HiltViewModel
class ProfileReturnViewModel @Inject constructor(): ClientViewModel<ProfileReturnIntent, ProfileReturnModel, Event>(
    ProfileReturnModel()
) {

    override fun dispatch(intent: ProfileReturnIntent) {
        when (intent) {
            is ProfileReturnIntent.BackClick -> launch { ProfileStackEventManager.send(BackRoute) }
        }
    }
}
