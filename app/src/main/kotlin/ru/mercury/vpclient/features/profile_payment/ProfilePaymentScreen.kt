...
fun ProfilePaymentScreen(
    viewModel: ProfilePaymentViewModel = hiltViewModel()
) {
    val state by viewModel.stateFlow.collectAsStateWithLifecycle()

    ProfilePaymentScreenContent(
        state = state,
...
@Composable
private fun ProfilePaymentScreenContent(
    state: ProfilePaymentModel,
    dispatch: (ProfilePaymentIntent) -> Unit
) {
    SharedScaffold(
        topBar = {
            CenterAlignedTopAppBar(
                ...
                navigationIcon = {
                    IconButton(
                        onClick = { dispatch(ProfilePaymentIntent.BackClick) }
                    ) {
                        Icon(
...
@Preview
@Composable
private fun ProfilePaymentScreenContentPreview() {
    ProfilePaymentScreenContent(
        state = ProfilePaymentModel(),
        dispatch = {}
    )
