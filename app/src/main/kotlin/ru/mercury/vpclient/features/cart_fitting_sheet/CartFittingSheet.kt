...
import ru.mercury.vpclient.shared.data.PREFIX_ASTERISK
...
fun CartFittingSheet(
    state: CartFittingModel,
    dispatch: (CartFittingIntent) -> Unit
) {
    SharedModalBottomSheet(
        onDismissRequest = { dispatch(CartFittingIntent.DismissClick) }
    ) {
        ...
        SharedColumn(
            ...
            CenterAlignedTopAppBar(
                ...
                navigationIcon = {
                    IconButton(
                        onClick = { dispatch(CartFittingIntent.DismissClick) }
                    ) {
                        Icon(
                            imageVector = Close24,
            ...
            Text(
                text = buildAnnotatedString {
                    append(stringResource(ClientStrings.CartFittingSheetFittingTabHintPrefix))
                    append(PREFIX_SPACE)
                    withStyle(style = SpanStyle(color = MaterialTheme.colorScheme.error)) {
                        append(stringResource(ClientStrings.CartFittingSheetFittingTabHintAccent))
                    }
                    append(stringResource(ClientStrings.CartFittingSheetFittingTabHintSuffix))
                },
                modifier = Modifier
            ...
            if (state.hasProductsWithoutSize) {
                Text(
                    text = buildAnnotatedString {
                        withStyle(style = SpanStyle(color = MaterialTheme.colorScheme.error)) {
                            append(PREFIX_ASTERISK)
                        }
                        append(PREFIX_SPACE)
                        append(stringResource(ClientStrings.CartFittingSheetNoSizeHint))
                    },
                    modifier = Modifier
                        .fillMaxWidth()
            ...
            Button(
                onClick = { dispatch(CartFittingIntent.ConfirmClick(selectedOption)) },
                modifier = Modifier
                    .padding(start = 16.dp, top = 16.dp, end = 16.dp)
                    .fillMaxWidth()
                    .height(52.dp),
                ...
                Text(
                    text = when (selectedOption) {
                        ...
                        CartFittingSheetOption.Manual -> {
                            stringResource(ClientStrings.CartFittingSheetContinue)
                        }
                    },
...
private fun CartFittingSheetPreview(
    @PreviewParameter(CartFittingModelPreviewParameterProvider::class) state: CartFittingModel
) {
    Box(
...
private class CartFittingModelPreviewParameterProvider: PreviewParameterProvider<CartFittingModel> {
    override val values: Sequence<CartFittingModel> = sequenceOf(
        CartFittingModel(
