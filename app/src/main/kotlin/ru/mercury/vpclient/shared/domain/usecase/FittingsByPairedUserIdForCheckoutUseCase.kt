...
// fixme
class FittingsByPairedUserIdForCheckoutUseCase @Inject constructor(
    ...
    override suspend fun execute(params: CheckoutBonusType): FittingCheckoutData {
        val pairedUserId = settingsDataStore.getValue(PreferenceKey.PairedUser)
            ...
            ?: throw FittingsByPairedUserIdForCheckoutException("Не удалось определить пользователя")
        val response = handleResponseResult(
            request = {
                val request = FittingsForCheckoutRequest(
                    bonusType = params
                )
                networkService.fittingsByPairedUserIdForCheckout(
            ...
            when (throwable) {
                ...
                else -> throw FittingsByPairedUserIdForCheckoutException(throwable.message.orEmpty())
            }
        }
        return response.entity
    }

    data class FittingsByPairedUserIdForCheckoutException(
