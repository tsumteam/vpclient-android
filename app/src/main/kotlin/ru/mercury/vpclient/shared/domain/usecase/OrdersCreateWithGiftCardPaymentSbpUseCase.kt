...
class OrdersCreateWithGiftCardPaymentSbpUseCase @Inject constructor(
    ...
    override suspend fun execute(paymentData: GiftCardPaymentData): GiftCardPaymentLinkData {
        ...
        val order = handleResponseResult(
            ...
            when (throwable) {
                ...
                else -> throw OrdersCreateWithGiftCardPaymentSbpException(throwable.message.orEmpty())
            }
        }
        val orderNumber = order.orderNumber?.takeIf { value -> value.isNotEmpty() }
            ?: throw OrdersCreateWithGiftCardPaymentSbpException("Не удалось определить заказ")
        val paymentLink = handleResponseResult {
            networkService.ordersByOrderIdPaymentSbp(orderNumber)
        }.getOrElse { throwable ->
            when (throwable) {
                is CancellationException -> throw throwable
                else -> throw OrdersCreateWithGiftCardPaymentSbpException(throwable.message.orEmpty())
        ...
        val url = paymentLink.qrCodeUrl?.takeIf { value -> value.isNotEmpty() }
            ?: throw OrdersCreateWithGiftCardPaymentSbpException("Не удалось получить ссылку на оплату")
    ...
    data class OrdersCreateWithGiftCardPaymentSbpException(
