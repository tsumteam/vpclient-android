package ru.mercury.vpclient.shared.ui.theme

import ru.mercury.vpclient.R

object ClientStrings {
    val AppName get() = R.string.app_name

    val CommonRetry get() = R.string.common_retry
    val CommonReset get() = R.string.common_reset

    val HomeGiftCards get() = R.string.home_gift_cards
    val HomeWatchMore get() = R.string.home_watch_more

    val SearchPlaceholder get() = R.string.search_placeholder
    val SearchCancel get() = R.string.search_cancel
    val SearchRecentlySearchedCaps get() = R.string.search_recently_searched_caps
    val SearchClear get() = R.string.search_clear
    val SearchRecentlyViewedCaps get() = R.string.search_recently_viewed_caps

    val GiftCardTitle get() = R.string.gift_card_title
    val GiftCardAmount get() = R.string.gift_card_amount
    val GiftCardAmountRange get() = R.string.gift_card_amount_range
    val GiftCardTerms get() = R.string.gift_card_terms
    val GiftCardTermsTitle get() = R.string.gift_card_terms_title
    val GiftCardBuy get() = R.string.gift_card_buy
    val GiftCardCheckoutTitle get() = R.string.gift_card_checkout_title
    val GiftCardCheckoutEmailPlaceholder get() = R.string.gift_card_checkout_email_placeholder
    val GiftCardCheckoutEmailHint get() = R.string.gift_card_checkout_email_hint
    val GiftCardCheckoutEmailError get() = R.string.gift_card_checkout_email_error
    val GiftCardCheckoutPhonePlaceholder get() = R.string.gift_card_checkout_phone_placeholder
    val GiftCardCheckoutPhoneHint get() = R.string.gift_card_checkout_phone_hint
    val GiftCardCheckoutPhoneError get() = R.string.gift_card_checkout_phone_error
    val GiftCardCheckoutDeliveryDate get() = R.string.gift_card_checkout_delivery_date
    val GiftCardCheckoutTotal get() = R.string.gift_card_checkout_total
    val GiftCardCheckoutPayByCard get() = R.string.gift_card_checkout_pay_by_card
    val GiftCardCheckoutPayBySbp get() = R.string.gift_card_checkout_pay_by_sbp
    val GiftCardCheckoutToday get() = R.string.gift_card_checkout_today
    val GiftCardCheckoutTomorrow get() = R.string.gift_card_checkout_tomorrow
    val GiftCardCheckoutNow get() = R.string.gift_card_checkout_now
    val GiftCardResultSuccessTitle get() = R.string.gift_card_result_success_title
    val GiftCardResultNotPaidTitle get() = R.string.gift_card_result_not_paid_title
    val GiftCardResultSuccessOrder get() = R.string.gift_card_result_success_order
    val GiftCardResultSuccessDate get() = R.string.gift_card_result_success_date
    val GiftCardResultSuccessEmail get() = R.string.gift_card_result_success_email
    val GiftCardResultSuccessPin get() = R.string.gift_card_result_success_pin
    val GiftCardResultSuccessInfo get() = R.string.gift_card_result_success_info
    val GiftCardResultNotPaidInfo get() = R.string.gift_card_result_not_paid_info
    val GiftCardResultPurchases get() = R.string.gift_card_result_purchases
    val GiftCardResultHome get() = R.string.gift_card_result_home

    val AgreementPersonalData get() = R.string.agreement_personal_data
    val AgreementTermsOfUse get() = R.string.agreement_terms_of_use
    val AgreementPrivacyPolicy get() = R.string.agreement_privacy_policy

    val WelcomeTitle get() = R.string.welcome_title
    val WelcomeDescription get() = R.string.welcome_description
    val WelcomeRegister get() = R.string.welcome_register
    val WelcomeLogin get() = R.string.welcome_login

    val RegisterTitle get() = R.string.register_title
    val RegisterNameLabel get() = R.string.register_name_label
    val RegisterPhoneLabel get() = R.string.register_phone_label
    val RegisterNameEmptyError get() = R.string.register_name_empty_error
    val RegisterPhoneEmptyError get() = R.string.register_phone_empty_error
    val RegisterPhoneInvalidError get() = R.string.register_phone_invalid_error
    val RegisterButton get() = R.string.register_button
    val RegisterAgreementText get() = R.string.register_agreement_text

    val LoginTitle get() = R.string.login_title
    val LoginPhoneLabel get() = R.string.login_phone_label
    val LoginPhoneEmptyError get() = R.string.login_phone_empty_error
    val LoginPhoneInvalidError get() = R.string.login_phone_invalid_error
    val LoginButton get() = R.string.login_button
    val LoginAgreementText get() = R.string.login_agreement_text

    val CodeTitle get() = R.string.code_title
    val CodeEmptyError get() = R.string.code_empty_error
    val CodeInvalidError get() = R.string.code_invalid_error
    val CodeButton get() = R.string.code_button
    val CodeSentDescription get() = R.string.code_sent_description
    val CodeResendCountdown get() = R.string.code_resend_countdown
    val CodeResendQuestion get() = R.string.code_resend_question
    val CodeResendButton get() = R.string.code_resend_button

    val MainTabHome get() = R.string.main_tab_home
    val MainTabBrands get() = R.string.main_tab_brands
    val MainTabCatalog get() = R.string.main_tab_catalog
    val MainTabFitting get() = R.string.main_tab_fitting
    val MainTabConsultants get() = R.string.main_tab_consultants
    val MainTabProfile get() = R.string.main_tab_profile

    val CatalogWatchAll get() = R.string.catalog_watch_all
    val CatalogViewAll get() = R.string.catalog_view_all
    val CatalogViewAllClothing get() = R.string.catalog_view_all_clothing
    val CatalogViewAllShoes get() = R.string.catalog_view_all_shoes
    val CatalogViewAllBags get() = R.string.catalog_view_all_bags
    val CatalogViewAllAccessories get() = R.string.catalog_view_all_accessories
    val CatalogViewAllBaby get() = R.string.catalog_view_all_baby
    val CatalogAddedToCart get() = R.string.catalog_added_to_cart

    val CartTitle get() = R.string.cart_title
    val CartToolbarCart get() = R.string.cart_toolbar_cart
    val CartToolbarFitting get() = R.string.cart_toolbar_fitting
    val CartAllItems get() = R.plurals.cart_all_items
    val CartPaymentItems get() = R.plurals.cart_payment_items
    val CartChat get() = R.string.cart_chat
    val CartFitting get() = R.string.cart_fitting
    val CartBuy get() = R.string.cart_buy
    val CartEmptyOrderTitle get() = R.string.cart_empty_order_title
    val CartEmptyOrderMessage get() = R.string.cart_empty_order_message
    val CartEmptyOrderButton get() = R.string.cart_empty_order_button
    val CartFittingEmptyOrderMessage get() = R.string.cart_fitting_empty_order_message
    val CartFittingEmptyOrderButton get() = R.string.cart_fitting_empty_order_button
    val CartArticle get() = R.string.cart_article
    val CartColor get() = R.string.cart_color
    val CartSize get() = R.string.cart_size
    val CartSelectSize get() = R.string.cart_select_size
    val CartSelectSizeCaps get() = R.string.cart_select_size_caps
    val CartSelectAlternative get() = R.string.cart_select_alternative
    val CartAlternativesNotFound get() = R.string.cart_alternatives_not_found
    val CartHideAlternatives get() = R.string.cart_hide_alternatives
    val CartSelectSizeForPaymentButton get() = R.string.cart_select_size_for_payment_button
    val CartSizeSheetSelect get() = R.string.cart_size_sheet_select
    val CartSizeSheetSelectForAlternative get() = R.string.cart_size_sheet_select_for_alternative
    val CartLookDefaultTitle get() = R.string.cart_look_default_title
    val CartLookAdd get() = R.string.cart_look_add
    val CartBuyItem get() = R.string.cart_buy_item
    val CartSold get() = R.string.cart_sold
    val CartInStock get() = R.string.cart_in_stock
    val CartEdit get() = R.string.cart_edit
    val CartEditAddSize get() = R.string.cart_edit_add_size
    val CartEditSelectSize get() = R.string.cart_edit_select_size
    val CartEditChangeQuantity get() = R.string.cart_edit_change_quantity
    val CartEditChangeQuantityCaps get() = R.string.cart_edit_change_quantity_caps
    val CartEditChangeColor get() = R.string.cart_edit_change_color
    val CartNoAvailableColors get() = R.string.cart_no_available_colors
    val CartEditCancel get() = R.string.cart_edit_cancel
    val CartSave get() = R.string.cart_save
    val CartFittingReturnToBasket get() = R.string.cart_fitting_return_to_basket
    val CartFittingSelectColorCaps get() = R.string.cart_fitting_select_color_caps
    val CartDelete get() = R.string.cart_delete
    val CartDisassembleLook get() = R.string.cart_disassemble_look
    val CartDetachFromLook get() = R.string.cart_detach_from_look
    val CartReturnOriginal get() = R.string.cart_return_original
    val CartShowAlternatives get() = R.string.cart_show_alternatives
    val CartHideAlternativesList get() = R.string.cart_hide_alternatives_list
    val CartInBasket get() = R.string.cart_in_basket
    val CartToFitting get() = R.string.cart_to_fitting
    val CartToPayment get() = R.string.cart_to_payment
    val CartRedeemUntil get() = R.string.cart_redeem_until
    val CartConsultantName get() = R.string.cart_consultant_name
    val CartConsultantBrand get() = R.string.cart_consultant_brand
    val CartEmptyMessage get() = R.string.cart_empty_message
    val CartEmptyTabHint get() = R.string.cart_empty_tab_hint
    val CartFittingSheetTitle get() = R.string.cart_fitting_sheet_title
    val CartFittingSheetGreetingMale get() = R.string.cart_fitting_sheet_greeting_male
    val CartFittingSheetGreetingFemale get() = R.string.cart_fitting_sheet_greeting_female
    val CartFittingSheetDescription get() = R.string.cart_fitting_sheet_description
    val CartFittingSheetAllProducts get() = R.string.cart_fitting_sheet_all_products
    val CartFittingSheetPaymentProducts get() = R.string.cart_fitting_sheet_payment_products
    val CartFittingSheetManual get() = R.string.cart_fitting_sheet_manual
    val CartFittingSheetFittingTabHintPrefix get() = R.string.cart_fitting_sheet_fitting_tab_hint_prefix
    val CartFittingSheetFittingTabHintAccent get() = R.string.cart_fitting_sheet_fitting_tab_hint_accent
    val CartFittingSheetFittingTabHintSuffix get() = R.string.cart_fitting_sheet_fitting_tab_hint_suffix
    val CartFittingSheetNoSizeHint get() = R.string.cart_fitting_sheet_no_size_hint
    val CartFittingSheetConfirm get() = R.string.cart_fitting_sheet_confirm
    val CartFittingSheetContinue get() = R.string.cart_fitting_sheet_continue
    val FittingProductsSheetTitle get() = R.string.fitting_products_sheet_title
    val FittingProductsSheetSelect get() = R.string.fitting_products_sheet_select
    val FittingCompilationsEmptyMessage get() = R.string.fitting_compilations_empty_message
    val FittingCompilationDiscussInChat get() = R.string.fitting_compilation_discuss_in_chat
    val CompilationChatSheetTitle get() = R.string.compilation_chat_sheet_title
    val CompilationPreviewDiscussInChat get() = R.string.compilation_preview_discuss_in_chat
    val CompilationPreviewAddToBasketDescription get() = R.string.compilation_preview_add_to_basket_description
    val FittingCompilationLooks get() = R.plurals.fitting_compilation_looks
    val FittingCompilationThings get() = R.plurals.fitting_compilation_things

    val CheckoutYourBonuses get() = R.string.checkout_your_bonuses
    val CheckoutDeliveryPlaceTitle get() = R.string.checkout_delivery_place_title
    val CheckoutAmountChangedTitle get() = R.string.checkout_amount_changed_title
    val CheckoutAmountChangedButton get() = R.string.checkout_amount_changed_button
    val CheckoutPaymentMethod get() = R.string.checkout_payment_method
    val CheckoutPayOnlineCaps get() = R.string.checkout_pay_online_caps
    val CheckoutPayAtCashDeskCaps get() = R.string.checkout_pay_at_cash_desk_caps
    val CheckoutPayWithBonuses get() = R.string.checkout_pay_with_bonuses
    val CheckoutAddLoyaltyCard get() = R.string.checkout_add_loyalty_card
    val CheckoutOrder get() = R.string.checkout_order
    val CheckoutItemsCount get() = R.string.checkout_items_count
    val CheckoutPromotionDiscount get() = R.string.checkout_promotion_discount
    val CheckoutBonuses get() = R.string.checkout_bonuses
    val CheckoutTotal get() = R.string.checkout_total
    val CheckoutPayByCard get() = R.string.checkout_pay_by_card
    val CheckoutPayBySbp get() = R.string.checkout_pay_by_sbp
    val CheckoutConfirmOrder get() = R.string.checkout_confirm_order
    val CheckoutBonusSheetTitleCaps get() = R.string.checkout_bonus_sheet_title_caps
    val CheckoutPaymentMethodSheetTitleCaps get() = R.string.checkout_payment_method_sheet_title_caps
    val CheckoutPaymentMethodSheetAddCard get() = R.string.checkout_payment_method_sheet_add_card
    val CheckoutPaymentMethodSheetPay get() = R.string.checkout_payment_method_sheet_pay
    val CheckoutPaymentMethodSheetCard get() = R.string.checkout_payment_method_sheet_card
    val CheckoutBankCardSheetTitle get() = R.string.checkout_bank_card_sheet_title
    val CheckoutBankCardSheetNumberPlaceholder get() = R.string.checkout_bank_card_sheet_number_placeholder
    val CheckoutBankCardSheetExpirationPlaceholder get() = R.string.checkout_bank_card_sheet_expiration_placeholder
    val CheckoutBankCardSheetCvvPlaceholder get() = R.string.checkout_bank_card_sheet_cvv_placeholder
    val CheckoutBankCardSheetSave get() = R.string.checkout_bank_card_sheet_save
    val CheckoutBankCardSheetPay get() = R.string.checkout_bank_card_sheet_pay
    val CheckoutBankCardSheetError get() = R.string.checkout_bank_card_sheet_error
    val CheckoutSbpBankSheetTitle get() = R.string.checkout_sbp_bank_sheet_title
    val CheckoutSbpBankSheetSubtitle get() = R.string.checkout_sbp_bank_sheet_subtitle
    val CheckoutPaymentResultSuccessTitle get() = R.string.checkout_payment_result_success_title
    val CheckoutPaymentResultOrderedTitle get() = R.string.checkout_payment_result_ordered_title
    val CheckoutPaymentResultSuccessDeliveryPrefix get() = R.string.checkout_payment_result_success_delivery_prefix
    val CheckoutPaymentResultSuccessAddressPrefix get() = R.string.checkout_payment_result_success_address_prefix
    val CheckoutPaymentResultSuccessSuffix get() = R.string.checkout_payment_result_success_suffix
    val CheckoutPaymentResultSuccessInfo get() = R.string.checkout_payment_result_success_info
    val CheckoutPaymentResultUnpaidTitle get() = R.string.checkout_payment_result_unpaid_title
    val CheckoutPaymentResultUnpaidPrefix get() = R.string.checkout_payment_result_unpaid_prefix
    val CheckoutPaymentResultUnpaidAccent get() = R.string.checkout_payment_result_unpaid_accent
    val CheckoutPaymentResultUnpaidSuffix get() = R.string.checkout_payment_result_unpaid_suffix
    val CheckoutPaymentResultErrorTitle get() = R.string.checkout_payment_result_error_title
    val CheckoutPaymentResultErrorInfo get() = R.string.checkout_payment_result_error_info
    val CheckoutPaymentResultPurchases get() = R.string.checkout_payment_result_purchases
    val CheckoutPaymentResultCatalog get() = R.string.checkout_payment_result_catalog

    val FittingConfirmationTitle get() = R.string.fitting_confirmation_title
    val FittingConfirmationProductsTitle get() = R.plurals.fitting_confirmation_products_title
    val FittingConfirmationPlaceTitle get() = R.string.fitting_confirmation_place_title
    val FittingConfirmationPlaceValue get() = R.string.fitting_confirmation_place_value
    val FittingConfirmationPlaceHint get() = R.string.fitting_confirmation_place_hint
    val FittingConfirmationPlaceBoutique get() = R.string.fitting_confirmation_place_boutique
    val FittingConfirmationPlaceHome get() = R.string.fitting_confirmation_place_home
    val FittingConfirmationPlaceOther get() = R.string.fitting_confirmation_place_other
    val FittingConfirmationSelectAddress get() = R.string.fitting_confirmation_select_address
    val FittingConfirmationAddAddress get() = R.string.fitting_confirmation_add_address
    val FittingAddressSave get() = R.string.fitting_address_save
    val FittingAddressEdit get() = R.string.fitting_address_edit
    val FittingAddressDelete get() = R.string.fitting_address_delete
    val FittingAddressFormTitle get() = R.string.fitting_address_form_title
    val FittingAddressNewDeliveryTitle get() = R.string.fitting_address_new_delivery_title
    val FittingAddressCityStreetHousePlaceholder get() = R.string.fitting_address_city_street_house_placeholder
    val FittingAddressFlatOfficeLabel get() = R.string.fitting_address_flat_office_label
    val FittingAddressEntranceLabel get() = R.string.fitting_address_entrance_label
    val FittingAddressIntercomLabel get() = R.string.fitting_address_intercom_label
    val FittingAddressFloorLabel get() = R.string.fitting_address_floor_label
    val FittingAddressCommentPlaceholder get() = R.string.fitting_address_comment_placeholder
    val FittingAddressDeleteTitle get() = R.string.fitting_address_delete_title
    val FittingAddressDeleteMessage get() = R.string.fitting_address_delete_message
    val FittingAddressCancel get() = R.string.fitting_address_cancel
    val FittingAddressSelectDelivery get() = R.string.fitting_address_select_delivery
    val FittingConfirmationMultipleDeliveryInfo get() = R.string.fitting_confirmation_multiple_delivery_info
    val FittingConfirmationMultipleDeliveries get() = R.string.fitting_confirmation_multiple_deliveries
    val FittingConfirmationSingleDelivery get() = R.string.fitting_confirmation_single_delivery
    val FittingConfirmationChangeTime get() = R.string.fitting_confirmation_change_time
    val FittingConfirmationConfirmTime get() = R.string.fitting_confirmation_confirm_time
    val FittingConfirmationIntervalsEmpty get() = R.string.fitting_confirmation_intervals_empty
    val FittingConfirmationDeliveryProductsCount get() = R.plurals.fitting_confirmation_delivery_products_count
    val FittingConfirmationDeliveryTitle get() = R.string.fitting_confirmation_delivery_title
    val FittingConfirmationDeliveryValue get() = R.string.fitting_confirmation_delivery_value
    val FittingConfirmationDeliveryHint get() = R.string.fitting_confirmation_delivery_hint
    val FittingConfirmationCommentTitle get() = R.string.fitting_confirmation_comment_title
    val FittingConfirmationCommentPlaceholder get() = R.string.fitting_confirmation_comment_placeholder
    val FittingConfirmationTotalTitle get() = R.string.fitting_confirmation_total_title
    val FittingConfirmationConfirm get() = R.string.fitting_confirmation_confirm
    val FittingSuccessThanks get() = R.string.fitting_success_thanks
    val FittingSuccessDeliveryPrefix get() = R.string.fitting_success_delivery_prefix
    val FittingSuccessDeliveryDate get() = R.string.fitting_success_delivery_date
    val FittingSuccessDeliveryProducts get() = R.string.fitting_success_delivery_products
    val FittingSuccessDeliveryAddressPrefix get() = R.string.fitting_success_delivery_address_prefix
    val FittingSuccessDeliveryAddress get() = R.string.fitting_success_delivery_address
    val FittingSuccessInfo get() = R.string.fitting_success_info
    val FittingSuccessFitting get() = R.string.fitting_success_fitting
    val FittingSuccessCatalog get() = R.string.fitting_success_catalog
    val FittingDeliveryDelivered get() = R.string.fitting_delivery_delivered
    val FittingInfoTitle get() = R.string.fitting_info_title
    val FittingInfoAddressLabel get() = R.string.fitting_info_address_label
    val FittingInfoDeliveryTimeLabel get() = R.string.fitting_info_delivery_time_label
    val FittingInfoAssemblyMethodLabel get() = R.string.fitting_info_assembly_method_label
    val FittingInfoDeliveryMethodLabel get() = R.string.fitting_info_delivery_method_label
    val FittingInfoSalesRepresentativeLabel get() = R.string.fitting_info_sales_representative_label
    val FittingInfoPhoneLabel get() = R.string.fitting_info_phone_label
    val FittingInfoVehicleNumberLabel get() = R.string.fitting_info_vehicle_number_label
    val FittingInfoLogisticsService get() = R.string.fitting_info_logistics_service
    val FittingInfoSalesRepresentative get() = R.string.fitting_info_sales_representative
    val FittingInfoPhone get() = R.string.fitting_info_phone
    val FittingInfoVehicleNumber get() = R.string.fitting_info_vehicle_number

    val FilterProductsQuantity get() = R.plurals.filter_products_quantity
    val FilterShowProductsQuantity get() = R.plurals.filter_show_products_quantity
    val FilterSizeTitle get() = R.string.filter_size_title
    val FilterBrandTitle get() = R.string.filter_brand_title
    val FilterBrandSearchPlaceholder get() = R.string.filter_brand_search_placeholder
    val FilterBrandFavoritesHeader get() = R.string.filter_brand_favorites_header
    val FilterBrandTopHeader get() = R.string.filter_brand_top_header
    val FilterBrandAllHeader get() = R.string.filter_brand_all_header
    val BrandsFavoritesHeader get() = R.string.brands_favorites_header
    val FilterBrandSelectAll get() = R.string.filter_brand_select_all
    val FilterBrandEmptySearch get() = R.string.filter_brand_empty_search
    val FilterEmptyProductsMessage get() = R.string.filter_empty_products_message
    val FilterEmptyProductsButton get() = R.string.filter_empty_products_button

    val SortTitle get() = R.string.sort_title
    val SortSelect get() = R.string.sort_select
    val SortOurChoice get() = R.string.sort_our_choice
    val SortPriceAscending get() = R.string.sort_price_ascending
    val SortPriceDescending get() = R.string.sort_price_descending
    val SortArrivalDateDescending get() = R.string.sort_arrival_date_descending

    val SizeCountryRussia get() = R.string.size_country_russia
    val SizeCountryItaly get() = R.string.size_country_italy
    val SizeCountryFrance get() = R.string.size_country_france
    val SizeCountryInternational get() = R.string.size_country_international

    val DetailsWhatToWear get() = R.string.details_what_to_wear
    val DetailsAddToBasket get() = R.string.details_add_to_basket
    val DetailsPickAlternativeInBasket get() = R.string.details_pick_alternative_in_basket
    val DetailsSizeTitle get() = R.string.details_size_title
    val DetailsSizeTable get() = R.string.details_size_table
    val DetailsSizeSelect get() = R.string.details_size_select
    val DetailsSizeSelectCaps get() = R.string.details_size_select_caps
    val DetailsSizeInStock get() = R.string.details_size_in_stock
    val DetailsSold get() = R.string.details_sold
    val DetailsSizeSold get() = R.string.details_size_sold
    val DetailsOtherModelColors get() = R.string.details_other_model_colors
    val DetailsWearWithTitle get() = R.string.details_wear_with_title
    val DetailsMessageSheetTitle get() = R.string.details_message_sheet_title
    val DetailsMessageCommentPlaceholder get() = R.string.details_message_comment_placeholder
    val DetailsMessageProductArticle get() = R.string.details_message_product_article
    val DetailsCartAddedSheetTitle get() = R.string.details_cart_added_sheet_title
    val DetailsCartAddedSheetContinueShopping get() = R.string.details_cart_added_sheet_continue_shopping
    val DetailsCartAddedSheetCart get() = R.string.details_cart_added_sheet_cart
    val CompilationCartAddedSheetTitle get() = R.string.compilation_cart_added_sheet_title
    val CompilationCartAddedSheetReturnToCompilation get() = R.string.compilation_cart_added_sheet_return_to_compilation
    val CompilationCartAddedSheetCart get() = R.string.compilation_cart_added_sheet_cart
    val CompilationPreviewPromotionDetails get() = R.string.compilation_preview_promotion_details
    val CompilationPreviewPromotionText get() = R.string.compilation_preview_promotion_text
    val CompilationBenefitSheetTitle get() = R.string.compilation_benefit_sheet_title
    val CompilationBenefitSheetFullPrice get() = R.string.compilation_benefit_sheet_full_price
    val CompilationBenefitSheetDiscountPrice get() = R.string.compilation_benefit_sheet_discount_price
    val CompilationBenefitSheetBenefit get() = R.string.compilation_benefit_sheet_benefit
    val DetailsCompleteSetTitle get() = R.string.details_complete_set_title
    val DetailsFieldBrand get() = R.string.details_field_brand
    val DetailsFieldColor get() = R.string.details_field_color
    val DetailsFieldComposition get() = R.string.details_field_composition
    val DetailsFieldCountry get() = R.string.details_field_country
    val DetailsFieldItemId get() = R.string.details_field_item_id
    val DetailsFieldArticle get() = R.string.details_field_article
    val DetailsFieldMeasurements get() = R.string.details_field_measurements
    val DetailsArticleCopied get() = R.string.details_article_copied
    val DetailsManufacturerArticleCopied get() = R.string.details_manufacturer_article_copied

    val MessengerAttachGallery get() = R.string.messenger_attach_gallery
    val MessengerAttachCartProducts get() = R.string.messenger_attach_cart_products
    val MessengerAttachCatalog get() = R.string.messenger_attach_catalog
    val MessengerAttachCancel get() = R.string.messenger_attach_cancel
    val MessengerMessageEdited get() = R.string.messenger_message_edited
    val MessengerEditing get() = R.string.messenger_editing
    val MessengerMessagePlaceholder get() = R.string.messenger_message_placeholder
    val MessengerMessageReply get() = R.string.messenger_message_reply
    val MessengerMessageResend get() = R.string.messenger_message_resend
    val MessengerMessageCopy get() = R.string.messenger_message_copy
    val MessengerMessageEdit get() = R.string.messenger_message_edit
    val MessengerMessageDelete get() = R.string.messenger_message_delete
    val MessengerReplyYou get() = R.string.messenger_reply_you

    val VideoSpeedTitle get() = R.string.video_speed_title
    val VideoSpeed05 get() = R.string.video_speed_05
    val VideoSpeed1 get() = R.string.video_speed_1
    val VideoSpeed15 get() = R.string.video_speed_15
    val VideoSpeed2 get() = R.string.video_speed_2
    val VideoTime get() = R.string.video_time

    val ConsultantsTitle get() = R.string.consultants_title
    val ConsultantsActiveButton get() = R.string.consultants_active_button
    val ConsultantsMakeActiveButton get() = R.string.consultants_make_active_button
    val ConsultantActionCall get() = R.string.consultant_action_call
    val ConsultantActionFitting get() = R.string.consultant_action_fitting
    val ConsultantActionCart get() = R.string.consultant_action_cart
    val ConsultantActionChat get() = R.string.consultant_action_chat
    val ConsultantActionSelection get() = R.string.consultant_action_selection

    val ProfileTitle get() = R.string.profile_title
    val ProfileLogout get() = R.string.profile_logout
    val ProfileAddLoyaltyCard get() = R.string.profile_add_loyalty_card
    val ProfileLoyaltyCardTitle get() = R.string.profile_loyalty_card_title
    val ProfileLoyaltyCardBonuses get() = R.plurals.profile_loyalty_card_bonuses
    val ProfileLoyaltyCardQrCode get() = R.string.profile_loyalty_card_qr_code
    val ProfileLoyaltyCardMore get() = R.string.profile_loyalty_card_more
    val ProfileAlphaBankBannerBlackTitle get() = R.string.profile_alpha_bank_banner_black_title
    val ProfileAlphaBankBannerGoldTitle get() = R.string.profile_alpha_bank_banner_gold_title
    val ProfileAlphaBankBannerDescription get() = R.string.profile_alpha_bank_banner_description
    val ProfileAlphaBankMore get() = R.string.profile_alpha_bank_more
    val ProfileAlphaBankPrivilegesTitleCaps get() = R.string.profile_alpha_bank_privileges_title_caps
    val ProfileAlphaBankPrivilegeBlackBonus get() = R.string.profile_alpha_bank_privilege_black_bonus
    val ProfileAlphaBankPrivilegeBlackGoldBonus get() = R.string.profile_alpha_bank_privilege_black_gold_bonus
    val ProfileAlphaBankPrivilegePurchases get() = R.string.profile_alpha_bank_privilege_purchases
    val ProfileAlphaBankPrivilegePayment get() = R.string.profile_alpha_bank_privilege_payment
    val ProfileAlphaBankPrivilegeBlackService get() = R.string.profile_alpha_bank_privilege_black_service
    val ProfileAlphaBankPrivilegeBlackGoldService get() = R.string.profile_alpha_bank_privilege_black_gold_service
    val LoyaltyAddCardTitleCaps get() = R.string.loyalty_add_card_title_caps
    val LoyaltyAddCardPhoneModeCaps get() = R.string.loyalty_add_card_phone_mode_caps
    val LoyaltyAddCardCardModeCaps get() = R.string.loyalty_add_card_card_mode_caps
    val LoyaltyAddCardPhoneLabel get() = R.string.loyalty_add_card_phone_label
    val LoyaltyAddCardCardPlaceholder get() = R.string.loyalty_add_card_card_placeholder
    val LoyaltyAddCardWrongPhoneError get() = R.string.loyalty_add_card_wrong_phone_error
    val LoyaltyAddCardGenericError get() = R.string.loyalty_add_card_generic_error
    val LoyaltyCodeTitleCaps get() = R.string.loyalty_code_title_caps
    val LoyaltyCodePlaceholder get() = R.string.loyalty_code_placeholder
    val LoyaltyCodeSentCard get() = R.string.loyalty_code_sent_card
    val LoyaltyCodeResendCountdown get() = R.string.loyalty_code_resend_countdown
    val LoyaltyCodeResendQuestion get() = R.string.loyalty_code_resend_question
    val LoyaltyCodeResendButton get() = R.string.loyalty_code_resend_button
    val ProfileLoyaltyConfirm get() = R.string.profile_loyalty_confirm
    val ProfileLoyaltyQrDescription get() = R.string.profile_loyalty_qr_description
    val ProfileLoyaltyInfoTitle get() = R.string.profile_loyalty_info_title
    val ProfileLoyaltyInfoRulesCaps get() = R.string.profile_loyalty_info_rules_caps
    val ProfileLoyaltyInfoCardType get() = R.string.profile_loyalty_info_card_type
    val ProfileLoyaltyInfoTermsForObtaining get() = R.string.profile_loyalty_info_terms_for_obtaining
    val ProfileLoyaltyInfoValidity get() = R.string.profile_loyalty_info_validity
    val ProfileLoyaltyInfoRenewalTerms get() = R.string.profile_loyalty_info_renewal_terms
    val ProfileLoyaltyInfoDash get() = R.string.profile_loyalty_info_dash
    val ProfileLoyaltyInfoUnlink get() = R.string.profile_loyalty_info_unlink
    val ProfileLoyaltyInfoTermsOfUse get() = R.string.profile_loyalty_info_terms_of_use
    val ProfileLoyaltyUnlinkTitle get() = R.string.profile_loyalty_unlink_title
    val ProfileLoyaltyUnlinkMessage get() = R.string.profile_loyalty_unlink_message
    val ProfileLoyaltyUnlinkCancel get() = R.string.profile_loyalty_unlink_cancel
    val ProfileLoyaltyUnlinkConfirm get() = R.string.profile_loyalty_unlink_confirm
    val ProfileMyData get() = R.string.profile_my_data
    val ProfilePurchases get() = R.string.profile_purchases
    val ProfileInformation get() = R.string.profile_information
    val ProfileQrCode get() = R.string.profile_qr_code
    val ProfileFavoriteBrands get() = R.string.profile_favorite_brands
    val ProfileBrandsWomanTabCaps get() = R.string.profile_brands_woman_tab_caps
    val ProfileBrandsManTabCaps get() = R.string.profile_brands_man_tab_caps
    val ProfileBrandsChildTabCaps get() = R.string.profile_brands_child_tab_caps
    val ProfileBrandsEmptyMessage get() = R.string.profile_brands_empty_message
    val ProfileBrandsAddFavoriteBrands get() = R.string.profile_brands_add_favorite_brands
    val ProfileBrandsChangeFavoriteBrands get() = R.string.profile_brands_change_favorite_brands
    val ProfileBrandsSave get() = R.string.profile_brands_save
    val ProfileBrandsEmptySearch get() = R.string.profile_brands_empty_search
    val ProfileRecentlyViewed get() = R.string.profile_recently_viewed
    val ProfileRecentlyViewedCaps get() = R.string.profile_recently_viewed_caps
    val ProfileWatchMore get() = R.string.profile_watch_more
    val ProfileLogoutTitle get() = R.string.profile_logout_title
    val ProfileLogoutMessage get() = R.string.profile_logout_message
    val ProfileLogoutCancel get() = R.string.profile_logout_cancel
    val ProfileOrdersNumber get() = R.string.profile_orders_number
    val ProfileOrdersReceiptNumber get() = R.string.profile_orders_receipt_number
    val ProfileOrdersAmount get() = R.string.profile_orders_amount
    val ProfileOrdersAwaitingPayment get() = R.string.profile_orders_awaiting_payment
    val ProfileOrdersMoreProducts get() = R.string.profile_orders_more_products
    val ProfileOrdersEmptyMessage get() = R.string.profile_orders_empty_message
    val ProfileOrderSubtitle get() = R.string.profile_order_subtitle
    val ProfileOrderSubtitleWithoutDate get() = R.string.profile_order_subtitle_without_date
    val ProfileOrderPaymentMethod get() = R.string.profile_order_payment_method
    val ProfileOrderPaymentOnReceipt get() = R.string.profile_order_payment_on_receipt
    val ProfileOrderPaymentAlert get() = R.string.profile_order_payment_alert
    val ProfileOrderPay get() = R.string.profile_order_pay
    val ProfileOrderPaymentAlertHours get() = R.plurals.profile_order_payment_alert_hours
    val ProfileOrderPaymentAlertMinutes get() = R.plurals.profile_order_payment_alert_minutes
    val ProfileOrderInfoTitle get() = R.string.profile_order_info_title
    val ProfileOrderProductQuantity get() = R.string.profile_order_product_quantity

    val ProfileInfoPayment get() = R.string.profile_info_payment
    val ProfileInfoDelivery get() = R.string.profile_info_delivery
    val ProfileInfoReturn get() = R.string.profile_info_return
    val ProfileInfoPrivacyPolicy get() = R.string.profile_info_privacy_policy
    val ProfileInfoGiftCard get() = R.string.profile_info_gift_card
    val ProfileInfoContacts get() = R.string.profile_info_contacts
    val ProfileContactsConsultantTitle get() = R.string.profile_contacts_consultant_title
    val ProfileContactsCustomerServiceTitle get() = R.string.profile_contacts_customer_service_title

    val ProfileQrTitle get() = R.string.profile_qr_title
    val ProfileQrDescription get() = R.string.profile_qr_description

    val MyDataTitle get() = R.string.my_data_title
    val MyDataSurname get() = R.string.my_data_surname
    val MyDataName get() = R.string.my_data_name
    val MyDataPhone get() = R.string.my_data_phone
    val MyDataEmail get() = R.string.my_data_email
    val MyDataNotSpecified get() = R.string.my_data_not_specified
    val MyDataDeleteProfile get() = R.string.my_data_delete_profile
    val MyDataDeleteDialogTitle get() = R.string.my_data_delete_dialog_title
    val MyDataDeleteDialogMessage get() = R.string.my_data_delete_dialog_message
    val MyDataDeleteCancel get() = R.string.my_data_delete_cancel
    val MyDataDeleteConfirm get() = R.string.my_data_delete_confirm

    val NotificationsTitle get() = R.string.notifications_title
    val NotificationsAllCaps get() = R.string.notifications_all_caps
    val NotificationsConsultantsCaps get() = R.string.notifications_consultants_caps
    val NotificationsCatalogsAndActionsCaps get() = R.string.notifications_catalogs_and_actions_caps
    val NotificationsEmpty get() = R.string.notifications_empty
    val NotificationsFilterEmpty get() = R.string.notifications_filter_empty
    val NotificationsYesterday get() = R.string.notifications_yesterday

    val PushNotificationsSheetTitle get() = R.string.push_notifications_sheet_title
    val PushNotificationsSheetDescription get() = R.string.push_notifications_sheet_description
    val PushNotificationsSheetEnable get() = R.string.push_notifications_sheet_enable
    val PushNotificationsSheetNotNow get() = R.string.push_notifications_sheet_not_now
}
