...
import ru.mercury.vpclient.core.ui.preview.FiltersRowStateProvider
...
fun FiltersRow(
    ...
    val selectedTopFilterValueChipsByParentChipId = topFilterChipIds.associateWith { chipId ->
        selectedFilterValueChipsByParentChipId[chipId].orEmpty()
    }
    val selectedStandaloneTopFilterValueChips = selectedStandaloneFilterValueChips
    val selectedTopFilterValueChipIds = selectedTopFilterValueChipsByParentChipId.values
        .flatten()
        .map(FilterChip::id)
        .plus(selectedStandaloneTopFilterValueChips.map(FilterChip::id))
        .toSet()
    val unselectedTopFilterValueChips = filterRibbonData.topFilterValueChips.filterNot { chip ->
        chip.id in selectedTopFilterValueChipIds
    }
    ...
    val topRowChipIds = buildList {
        add(FiltersRowState.SORT_CHIP_KEY)
        filterRibbonData.topFilterChips.forEach { chip ->
            add(chip.id)
            addAll(selectedTopFilterValueChipsByParentChipId[chip.id].orEmpty().map(FilterChip::id))
        }
        addAll(selectedStandaloneTopFilterValueChips.map(FilterChip::id))
        addAll(unselectedTopFilterValueChips.map(FilterChip::id))
        if (sortSelected || selectedFilterValueChips.isNotEmpty()) {
            add(FiltersRowState.RESET_CHIP_KEY)
        }
    }
            ...
            when {
                filterRibbonData.isEmpty -> {
                    ...
                    filterRibbonData.topFilterChips.forEach { chipData ->
                        ...
                        selectedTopFilterValueChipsByParentChipId[chipData.id].orEmpty().forEach { selectedChip ->
                            item(
                                key = selectedChip.id
                            ) {
                                FilterChip(
                    ...
                    items(
                        items = selectedStandaloneTopFilterValueChips,
                    ...
                    if (sortSelected || selectedFilterValueChips.isNotEmpty()) {
                        item(
                            key = FiltersRowState.RESET_CHIP_KEY
                        ) {
                            FilterResetChip(
                                enabled = enabled,
                                onClick = {
